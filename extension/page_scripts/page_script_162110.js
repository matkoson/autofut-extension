...
jsonEl.id = `${extensionName}-json-message`;
document.body.appendChild(jsonEl);

window.addEventListener(`${extensionName}-simulate-key-event`, function (e) {
    const editorEl = document.getElementById("waffle-rich-text-editor");
    const args = JSON.parse(jsonEl.textContent);
